...
from li2former.detector import Detector
from li2former.config import Config
...
class Li2FormerROS:
    def __init__(self):
        ...
        self.config = Config()
        self.config.load(rospy.get_param("~cfg"))
    ...
    def _scan_callback(self, msg):
        ...
        dets_xy, dets_cls = self._detector(scan)
        end_time = time.time()

        self.avg_fps = ((self.cnt - 1) * self.avg_fps + 1 / (end_time - start_time)) / self.cnt
        self.cnt += 1
    ...
    def publishPosMarker(self, msg, dets_xy):
        ...
        rviz_msg.color.r = 1.0
        rviz_msg.color.g = 0.0
